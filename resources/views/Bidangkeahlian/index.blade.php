...
<div class="card">
    ...
    <div class="card-body">
        ...
        @endif
        <form action="{{ route('bidangkeahlian.store') }}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                {{-- <div class="form-group">
                    <label for="exampleInputPassword1">Id</label>
                    <input type="text" class="form-control" id="exampleInputPassword1" name="id_jabatan">
                    @error('id_jabatan')
            ...
            @forelse ($bidangkeahlian as $row )
                    <tr>
                        ...
                        <td>
                            <div class="btn-group">
                                <div class="d-flex">
                                    <form action={{route('bidangkeahlian.destroy', ['id'=>$row->id])}} method="POST">
                                        @csrf
                                        @method('delete')
                                        <button class="btn btn-danger">hapus</button>
                                    </form>
                                    <a class="btn btn-warning" href={{route('bidangkeahlian.edit', ['id'=>$row->id])}}>ubah
                                    </a>
                                </div>
                            </div>
                        </td>
